...
@section('title','Blog Categories > Edit Blog Category')
@section('content')
				...
				<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
					...
					<form method="POST" action="{!! route('blog-categories.update',$blog_category->id) !!}">
							...
							<div class="col-sm-12 col-md-10">
								<select class="custom-select col-12" name="parent_id">
									...
									@foreach($blog_categories_parents as $key => $blog_categories_parent )
										<option value="{!! $key !!}" @if($blog_category->parent != '') @if($blog_category->parent->name == $blog_categories_parent) selected @endif @endif>{!! $blog_categories_parent !!}</option>
									@endforeach
								</select>
						...
						<div class="form-group row">
							...
							<div class="col-sm-12 col-md-10">
								<input class="form-control" type="text" placeholder="Johnny Brown" name="name" value="{{ $blog_category->name }}">
